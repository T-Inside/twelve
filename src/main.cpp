...
unsigned int nStakeMinAge = 60 * 60 * 4;	// minimum age for coin age:  12h
unsigned int nStakeMaxAge = -1;	// unlimited
unsigned int nStakeTargetSpacing = 60;			// 60 sec block spacing

int64 nChainStartTime = 1432726487;
...
const string strMessageMagic = "twelveSigned Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
 int64_t nSubsidy = 0 * COIN;
	...
	if(pindexBest->nHeight+1 >= 2 && pindexBest->nHeight+1 <= 100)
    {
		nSubsidy = 0 * COIN;  
    } 
	else if(pindexBest->nHeight+1 >= 101 && pindexBest->nHeight+1 <= 40000)
    {
		nSubsidy = 1 * COIN;  
    } 	
	else if(pindexBest->nHeight+1 >= 40001 && pindexBest->nHeight+1 <= 60000)
    {
		nSubsidy = 10 * COIN;  
    } 	
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
   int64_t nSubsidy = 0 * COIN;
	...
	if(pindexBest->nHeight+1 >= 7200 && pindexBest->nHeight+1 <= 8640)
    {
		nSubsidy = 2.5 * COIN;  
    } 
	else if(pindexBest->nHeight+1 >= 8641 && pindexBest->nHeight+1 <= 10080)
    {
		nSubsidy = 5 * COIN;  
    } 	
	else if(pindexBest->nHeight+1 >= 10081 && pindexBest->nHeight+1 <= 11520)
    {
		nSubsidy = 7.5 * COIN;  
    } 
	else if(pindexBest->nHeight+1 >= 11521 && pindexBest->nHeight+1 <= 12960)
    {
		nSubsidy = 50 * COIN;  
    } 
	else if(pindexBest->nHeight+1 >= 12961)
    {
		nSubsidy = nCoinAge * MAX_Twelve_PROOF_OF_STAKE / 365;
    } 
...
static const int64 nTargetTimespan = 60 * 15; // 1 blocks  
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Twelve
    bool fStrictPayToScriptHash = true; // Always active in Twelve
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Twelve", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "First X12 Algo Twelve!";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1432726487;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1293458;
        ...
        assert(block.hashMerkleRoot == uint256("0x158dd664b29e7dc28ad822ae75b8afc1d252170b5a3b3c401666624b4cf32603"));
